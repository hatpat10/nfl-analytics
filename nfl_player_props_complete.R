...
SEASON <- 2025
TRAIN_WEEKS <- 1:5  # Weeks to train on (weeks 1-5)
PREDICT_WEEK <- 6   # Week to predict
...
cat("Generating predictions for all betting markets...\n\n")

# Moneyline predictions (ensemble average)
pred_win_logit <- predict(model_logit, newdata = X_predict, type = "response")
pred_win_rf <- predict(model_rf_win, X_predict, type = "prob")[, "Win"]
pred_win_xgb <- predict(model_xgb_win, xgb.DMatrix(data = as.matrix(X_predict)))
pred_win_ensemble <- (pred_win_logit + pred_win_rf + pred_win_xgb) / 3

# Spread predictions (ensemble average)
pred_spread_lm <- predict(model_lm_spread, newdata = X_predict)
pred_spread_rf <- predict(model_rf_spread, X_predict)
pred_spread_xgb <- predict(model_xgb_spread, xgb.DMatrix(data = as.matrix(X_predict)))
pred_spread_ensemble <- (pred_spread_lm + pred_spread_rf + pred_spread_xgb) / 3

# Total predictions (ensemble average)
pred_total_lm <- predict(model_lm_total, newdata = X_predict)
pred_total_rf <- predict(model_rf_total, X_predict)
pred_total_xgb <- predict(model_xgb_total, xgb.DMatrix(data = as.matrix(X_predict)))
pred_total_ensemble <- (pred_total_lm + pred_total_rf + pred_total_xgb) / 3
...
if (file.exists(odds_file)) 
  ...
  if (is.na(matchup_col)) {
    ...
    # Export full comparison with Vegas
    write_xlsx(
      list(
